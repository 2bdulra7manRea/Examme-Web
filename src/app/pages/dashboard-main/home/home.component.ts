...
export class HomeComponent implements OnInit, OnDestroy {
  ...
  ngAfterViewInit(): void {
    ...
    this.httpService.getExams().subscribe((exams) => {
      ...
      this.reExmas = this.Exams.sort((a:any,b:any)=>{
        let aDate:any=new Date(a.timeCreated)
        let bDate:any=new Date(b.timeCreated)
        return bDate-aDate
      }).splice(0,5)
    })
  ...
  deleteMyRoom(itemId, uName) {
    let confirmFlag= confirm('Are you sure to delete the room ?')
    if(confirmFlag){
